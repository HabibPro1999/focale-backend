...
export async function deleteForm(id: string): Promise<void> {
  ...
  // Check if form has registrations - cannot delete if it does
  const regCount = await prisma.registration.count({ where: { formId: id } });
  if (regCount > 0) {
    throw new AppError(
      `Cannot delete form with ${regCount} registration(s). Deactivate the form instead.`,
      409,
      true,
      ErrorCodes.FORM_HAS_REGISTRATIONS,
    );
  }
