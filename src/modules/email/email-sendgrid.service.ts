...
export async function sendBatchEmails(
  ...
  for (let i = 0; i < emails.length; i += safeBatchSize) {
    ...
    try {
      // sgMail.send() with an array iterates and sends each message as a separate API call (v8.x)
      await sgMail.send(messages);

      // If no error thrown, assume all messages in the batch succeeded
...
export function verifyWebhookSignature(
  ...
  if (!SENDGRID_WEBHOOK_PUBLIC_KEY) {
    logger.error(
      "SendGrid webhook public key not configured - rejecting webhook",
    );
    return false;
  }
