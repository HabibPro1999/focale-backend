...
/**
 * Realtime event envelopes emitted after DB mutations and consumed by the SSE
 * route. Payloads carry only IDs and minimal fields — admins refetch the full
 * record through existing authenticated GET routes. Ported verbatim from the
 * legacy `src/core/events/types.ts`; lives in contracts because both the db
 * outbox layer (enqueueRealtimeOutboxEvent) and the api event bus consume it.
 */

/** GET /api/stream querystring: both optional, non-strict (SSE cache-busters). */
...
/**
 * Producers skip any debounce/coalescing for these two types — a check-in or
 * payment confirmation must always push, never be dropped by rapid-update
 * coalescing. The debounce logic itself lives in producer modules.
 */
