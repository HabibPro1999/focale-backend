...
// ============================================================================
// Access capacity accounting — OWNED BY access module.
// ponytail: NOTE FOR VERIFIER — getAlreadyCoveredAccessIds + syncPaidCountDelta
// ported from src/modules/access/access.service.ts (empty stub at port time).
// The paidCount delta core + capacity gate throw are kept faithfully; the
// capacity-reached auto-drop cascade (handleCapacityReached) is DEFERRED with the
// audit/outbox subsystems wave-1 also omitted. Relocate to queries/access.ts.
// ============================================================================
...
function paidAccessQuantities(
  ...
  if (!isFullySettled && status !== "PARTIAL") return quantities;
  const breakdown = priceBreakdown as AccessBreakdown | null;
...
/**
 * Thrown by the ported access-capacity helpers. Carries code/status so the API's
 * exception filter renders the legacy envelope. Kept db-package-local (the db
 * layer cannot import Nest). ponytail: fold into the real access module error
 * path when it lands.
 */
export class SponsorshipAccessError extends Error {
  constructor(
    ...
    super(message);
    this.name = "SponsorshipAccessError";
  }
}

async function incrementPaidCount(
  db: DbExecutor,
  accessId: string,
  quantity: number,
): Promise<void> {
  ...
  const [access] = await db
    ...
    .limit(1);
  if (!access) {
    throw new SponsorshipAccessError("Access not found", 404, "ACC_7001");
  }
  const remaining =
    access.maxCapacity === null
      ? null
      : Math.max(0, access.maxCapacity - access.paidCount);
  throw new SponsorshipAccessError(
...
async function decrementPaidCount(
  db: DbExecutor,
  accessId: string,
  quantity: number,
): Promise<void> {
  const result = await db
    .update(eventAccess)
    .set({ paidCount: sql`${eventAccess.paidCount} - ${quantity}` })
    .where(
      and(
        eq(eventAccess.id, accessId),
        sql`${eventAccess.paidCount} >= ${quantity}`,
      ),
    )
    .returning({ id: eventAccess.id });
...
export async function getAlreadyCoveredAccessIds(
  ...
  if (excludeSponsorshipId) {
    clauses.push(ne(sponsorshipUsages.sponsorshipId, excludeSponsorshipId));
  }
  const rows = await db
    ...
    .where(and(...clauses));
  return new Set(rows.flatMap((r) => r.coveredAccessIds ?? []));
}

export interface PaidCountState {
  status: string;
  priceBreakdown: unknown;
  coveredAccessIds?: Set<string>;
}
...
export async function syncPaidCountDelta(
  db: DbExecutor,
  _eventId: string,
  oldState: PaidCountState,
  newState: PaidCountState,
): Promise<void> {
  const oldPaid = paidAccessQuantities(
    oldState.status,
    oldState.priceBreakdown,
    oldState.coveredAccessIds ?? new Set<string>(),
  );
  const newPaid = paidAccessQuantities(
    newState.status,
    newState.priceBreakdown,
    newState.coveredAccessIds ?? new Set<string>(),
  );
  const accessIds = new Set([...oldPaid.keys(), ...newPaid.keys()]);
  for (const accessId of accessIds) {
    const delta = (newPaid.get(accessId) ?? 0) - (oldPaid.get(accessId) ?? 0);
    if (delta > 0) await incrementPaidCount(db, accessId, delta);
    else if (delta < 0) await decrementPaidCount(db, accessId, Math.abs(delta));
  }
  // ponytail: capacity-reached auto-drop cascade (handleCapacityReached) omitted —
  // it emits audit/outbox events, both deferred across this port wave.
}
