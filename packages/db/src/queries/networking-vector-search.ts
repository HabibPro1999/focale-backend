...
const candidate = alias(networkingProfiles, "p");
const candidateRegistration = alias(registrations, "r");
...
export async function rankNetworkingVectorCandidates(
  ...
  const rows = rowsOf<{
    ...
    await db.execute(sql`
    WITH source AS (
      SELECT p.id,p.offers,p.seeks,pe.embedding AS profile,oe.embedding AS offer,ne.embedding AS need
      FROM networking_profiles p
      ...
      WHERE p.event_id=${eventId} ${
        candidateIds
          ...
          : sql``
      } AND ${discoverableCounterpart(candidate, candidateRegistration, paymentStatuses, { eventId, profileId })}
        AND ${notInteracted(eventId, profileId, candidate.id)}
    )
