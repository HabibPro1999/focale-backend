...
- Pending table requests hold inventory only, plus one pending-request slot per requester (`hold:profile:<requester>`, added later); people become booked at acceptance. Manual allocation keeps PENDING until the other participant accepts. Decline/cancellation/expiry releases holds. Confirmed reschedules retain the original booking until accepted.
