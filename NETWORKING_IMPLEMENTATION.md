...
- Pending meeting requests hold their table and one pending-request slot per requester (`hold:profile:<requester>`), never the participants, and expire. Acceptance reserves both participants and one table atomically. See "Meeting lifecycle" in NETWORKING.md. Fixed event slots and unique resource reservations prevent double bookings. Manual allocation uses an explicit pending-allocation state. Rescheduling preserves the old confirmed booking until replacement succeeds.
