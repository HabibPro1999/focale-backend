...
import { getDb, networkingStore, type NetworkingRow } from "@app/db";
...
describe.runIf(dbTestsEnabled())("networking writes under concurrency", () => {
  ...
  it("finishes 10 concurrent swipes by distinct participants", async () => {
    ...
    const likes = await networkingStore(getDb()).all("interests", { eventId: fixture.event.id, action: "LIKE" });
  ...
  it("turns 10 concurrent mutual likes into one connection and one MATCH per side", async () => {
    ...
    expect(await networkingStore(getDb()).all("connections", { eventId: fixture.event.id, profileAId, profileBId })).toHaveLength(1);
    for (const person of [a, b])
      expect(await networkingStore(getDb()).all("notifications", { eventId: fixture.event.id, profileId: person.profile.id, type: "MATCH" })).toHaveLength(1);
    expect(await networkingStore(getDb()).all("interests", { eventId: fixture.event.id, profileId: a.profile.id, targetId: b.profile.id })).toHaveLength(1);
  });
  ...
  it("finishes 10 concurrent messages on distinct connections", async () => {
    ...
    const connections = await Promise.all(range(10).map(async (i) => {
      ...
      return (await networkingStore(getDb()).one("connections", { eventId: fixture.event.id, profileAId, profileBId }))!;
    }));
    const results = await settle(range(10).map((i) => () =>
      social.sendMessage(people[i], connections[i].id, `Hello ${i}`, `message-${i}`)));
    expect(failures(results)).toEqual([]);
    expect(await networkingStore(getDb()).all("messages", { eventId: fixture.event.id })).toHaveLength(10);
  });
  ...
  it("finishes 10 concurrent sign-in code requests for distinct participants", async () => {
    ...
    expect(failures(results)).toEqual([]);
    const deliveries = await networkingStore(getDb()).all("deliveries", { eventId: fixture.event.id, type: "OTP" });
  ...
  it("books 10 concurrent requests onto 10 tables, then confirms them all concurrently", async () => {
    ...
    expect(await networkingStore(getDb()).all("meetings", { eventId: fixture.event.id, status: "CONFIRMED" })).toHaveLength(10);
  ...
  it("books 3 of 10 concurrent requests onto 3 tables and answers the other 7 with 409", async () => {
    for (const table of fixture.tables.slice(3))
      await networkingStore(getDb()).update("tables", { eventId: fixture.event.id, id: table.id }, { active: false });
    const results = await settle(range(10).map((i) => () =>
  ...
  it("keeps one participant from being confirmed twice when both of their slot requests are accepted at once", async () => {
    for (const table of fixture.tables.slice(3))
      await networkingStore(getDb()).update("tables", { eventId: fixture.event.id, id: table.id }, { active: true });
    const host = people[0];
