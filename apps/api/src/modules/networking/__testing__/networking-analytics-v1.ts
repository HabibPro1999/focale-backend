/**
 * The organizer analytics calculator as it was before plan 4.9, kept verbatim
 * as the golden-test oracle: it loads every networking row of the event and
 * computes in memory. `networking.analytics.db.test.ts` holds the SQL
 * aggregates (`networkingAnalytics`) to it on the same fixtures. Test-only.
 */
 ...
 */
export function calculateNetworkingAnalyticsV1(
...
export async function networkingAnalyticsV1(
  eventId: string,
  now = new Date(),
): Promise<
...
> {
  ...
  return {
    ...calculateNetworkingAnalyticsV1({
      profiles,
      interests,
      connections,
      messages,
